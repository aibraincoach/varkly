...
These operations record transitions without calling a reviewer. They preserve earlier attempts/captures; they do not auto-delete stop markers or assume an old reset estimate proves present availability. `abandon` explicitly closes failed or unstarted scope without erasing its history. A known still-running AGY owner cannot be overridden by recovery; stored process identity distinguishes PID reuse. For a changed hostname, inaccessible identity or interrupted PPLX harness, `--reconcile-owner` requires explicit PM evidence that the previous assignment is no longer active. This never sends a kill signal or automatically takes over another coder. Resume only missing packets, never completed coverage. Shared-account quota failures must also be reported to the other responsible PMs through an authorized coordination channel; this project-local ledger does not claim fleet-wide mutual exclusion.
